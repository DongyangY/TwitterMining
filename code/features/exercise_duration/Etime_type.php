...
for($j = 0; $j < count($Exe_type); $j++){
      ...
      $sql = "SELECT tweet_text, screen_name, geo_lat , geo_long, tweet_id FROM tweets WHERE tweet_text LIKE
      '%".$Exe_type[$j]."%' AND 
       tweet_text LIKE '%min%'"; 
 //echo $Exe_type[$j];
 //echo $date_array[$i];
    ...
    $sql = "INSERT INTO ExerciseTime(Exe_type,exercise_date,exercise_time) Values ('".$Exe_type[$j]."','"tweets.created_at"','"$result1"')";
        mysqli_query($con,$sql);
           ...
           if($num != 0 && $num < 200 ){
             ...
             $sql = "INSERT INTO Etime_type(screen_name, exercise_type,exercise_time, geo_lat, geo_long, tweet_id) Values ('".$array[1]."','".$Exe_type[$j]."','".$num."',".$array[2].",".$array[3].",".$array[4].")";
              echo"int is :". $num."<br>";
            } 
