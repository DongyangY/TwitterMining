...
$sql_delete = "DELETE FROM EI_type_state_mood";
...
for($q=0;$q<count($exercise_type);$q++){   //loop exercise type
  for($p=0;$p<count($state_name);$p++){  //loop date

    $k4=0;
    $sql4 = "SELECT tweet_text FROM tweets LEFT JOIN users on 
    tweets.user_id = users.user_id WHERE location LIKE 
    '%".$state_name[$p]."%'AND tweet_text LIKE '%".$exercise_type[$q]."%'";
...
//通过$desp_key这里计算的是当前条目的评分数
    ...
    //echo "effect k5 is :".$Effect_k5."<br>";
     $final = $s5/$Effect_k5;
     echo $final;
     $sql = "INSERT INTO EI_type_state_mood(type,state,Mood_Value) VALUES ('".$exercise_type[$q]."','".$state_name[$p]."',$final)";
