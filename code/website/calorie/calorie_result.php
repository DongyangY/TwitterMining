...
mysqli_query($con,"DELETE FROM calorie_result");
...
$exercise_type = array("running","cycling","swimming","basketball","football");
...
for($i = 0; $i < count($exercise_type); $i++){
    ...
    for($j = 0; $j < count($time_array); $j++){
        ...
        $sql = "SELECT AVG(exercise_time) FROM calorie WHERE exercise_type = '".$exercise_type[$i]."' AND created_at LIKE '".$time_array[$j]."' AND (created_at LIKE '".$date_array[0]."' OR created_at LIKE '".$date_array[1]."' OR created_at LIKE '".$date_array[2]."' OR created_at LIKE '".$date_array[3]."' OR created_at LIKE '".$date_array[4]."')";
...
for($i = 0; $i < count($exercise_type); $i++){
    ...
    for($j = 0; $j < count($time_array); $j++){
        ...
        $sql = "SELECT AVG(exercise_time) FROM calorie WHERE exercise_type = '".$exercise_type[$i]."' AND created_at LIKE '".$time_array[$j]."' AND (created_at LIKE '".$date_array[5]."' OR created_at LIKE '".$date_array[6]."')";
...
for($j = 0; $j < count($time_array); $j++){
    $sql = "INSERT INTO calorie_result(workday, time, calorie) VALUES ( 1,".$j.",".$workday_caloire[$j].")";
    mysqli_query($con,$sql);
}

for($j = 0; $j < count($time_array); $j++){
    $sql = "INSERT INTO calorie_result(workday, time, calorie) VALUES ( 0,".$j.",".$weekday_caloire[$j].")";
    mysqli_query($con,$sql);
}
